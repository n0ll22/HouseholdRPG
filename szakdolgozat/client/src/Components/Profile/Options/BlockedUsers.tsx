    ...
    <div className="py-10">
      ...
      <div className="space-y-2">
        ...
        {blockedUsers.length > 0 &&
          blockedUsers.filter((f) => f.blockedBy === loggedInUserId) &&
          blockedUsers
            // csak olyan blokkolt felhasználók, akiket a felhasználó blokkolt
            .map((f) => (
              <div className="flex items-center">
                <img
                  className="w-10 h-10 rounded-md mr-2"
                  src={`/img/pfps/${f.otherUser.avatar}`}
                  alt=""
                />
                <p className="w-64">{f.otherUser.username}</p>
                {f.blockedBy === loggedInUserId ? (
                  <FaTrash
